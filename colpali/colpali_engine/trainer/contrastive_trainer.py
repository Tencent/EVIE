...
def concat_all_gather_nograd(t: torch.Tensor) -> torch.Tensor:
    """Gather an integer tensor across ranks in rank order (no autograd)."""
    if not (torch.distributed.is_available() and torch.distributed.is_initialized()):
        return t
    world = torch.distributed.get_world_size()
    buf = [torch.empty_like(t) for _ in range(world)]
    torch.distributed.all_gather(buf, t.contiguous())
...
class ContrastiveTrainer(Trainer):
    ...
    def _compute_loss_from_outputs(
        ...
        group_ids_local = group_ids
        group_ids_global = group_ids
        # Gather every microstep. `sync_gradients` is only True on the last
        # grad-accum step; gating on it shrinks in-batch CE to the local batch
        # for most steps (multi-GPU + grad accumulation).
        if self.accelerator.num_processes > 1:
            ...
            if group_ids is not None:
                group_ids_global = concat_all_gather_nograd(group_ids)
        ...
        if neg_target_outputs is not None:
            ...
            if listwise_grades is not None:
                loss_kwargs["listwise_grades"] = listwise_grades
            if slot_mask is not None:
                loss_kwargs["slot_mask"] = slot_mask
            if sample_weight is not None:
                loss_kwargs["sample_weight"] = sample_weight
            if group_ids is not None:
                loss_kwargs["group_ids_local"] = group_ids_local
                loss_kwargs["group_ids_global"] = group_ids_global
            loss = self.loss_func(
                **loss_kwargs,
            )
        else:
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # query -> doc loss
        listwise_grades = inputs.get(ColPaliEngineDataset.LISTWISE_GRADES_KEY)
        slot_mask = inputs.get(ColPaliEngineDataset.MASK_KEY)
        # Optional per-row weight and group id (all-pos shards). Absent on judged-pos batches.
        ...
        loss = self._compute_loss_from_outputs(
            ...
            doc_outputs,
            neg_doc_outputs,
            listwise_grades=listwise_grades,
            slot_mask=slot_mask,
